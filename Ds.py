...
class Matrix:
    ...
    def __init__(self, matrix):
        ...
        if len(set(map(len, matrix))) != 1:
            raise ValueError("!! Данная матрица не возможна !!")
    ...
    def __add__(self, other):
        ...
        if self.count_row != other.count_row or self.count_col != other.count_col:
            raise ValueError("!! Данные матрицы разных размеров !!")
        new_matrix = []
    ...
    def __mul__(self, other):
        ...
        if self.count_col != other.count_row:
            raise ValueError(f'!! Данные матрицы невозможно перемножить !!')
        new_matrix = []
...
if __name__ == '__main__':
    matrix1 = Matrix([[randint(1, 9) for _ in range(4)] for _ in range(3)])
